...
export const handleLogout = (): void => {
  ...
  if (isLocalStorageAvailable && userId) {
    localStorage.removeItem(`credits_${userId}`);
...
export const setCredits = (userId: string, credits: number): void => {
  if (isLocalStorageAvailable) {
    localStorage.setItem(`credits_${userId}`, String(credits));
  }
};

export const getCredits = (userId: string): number | null => {
  if (isLocalStorageAvailable) {
    const creditsString = localStorage.getItem(`credits_${userId}`);
    return creditsString ? parseInt(creditsString, 10) : 0;
  }
  return null;
};
